...
processDataAndFillGrid = function(dataToBeProcessed){
	...
	Ext.getCmp('tfDetailsRightArticleGrid').show();
	}
	
	//Intersection Store
	
	var intersectionSetList =dataToBeProcessed.instersectionSet;
	if(intersectionSetList){
	var lengthIntersectionSet =intersectionSetList.length;
	var intersectionSetArray = new Array();
	
	for(i=0;i<lengthIntersectionSet;i++){
		var intersectionSet=intersectionSetList[i]
		var dataTemp={'interSet':intersectionSet};
		intersectionSetArray.push(dataTemp);
	}
	
	var instersectionSetStore=Ext.getCmp('intersectionSetGrid').getStore();
	instersectionSetStore.loadData(intersectionSetArray);
	Ext.getCmp('intersectionSetGrid').show();
	}
	//unionSet Store
	...
	Ext.getCmp('rightArticleFreqInfoListGrid').show();
	}
...
tfDetailsLeftArticleGrid = Ext.create('Ext.grid.Panel', {
	...
	unionSetGrid = Ext.create('Ext.grid.Panel', {
		title:'Union Grid',
		forceFit : true,
		id : 'unionSetGrid',
		store : unionSetStore,
	...
	intersectionSetGrid = Ext.create('Ext.grid.Panel', {
	...
	leftArticleFreqInfoListGrid = Ext.create('Ext.grid.Panel', {
